...
- Do NOT add explanations"""
    ...
    try:
        resp = requests.post(url, json=payload, timeout=REQUEST_TIMEOUT_GEMINI)
...
def fake_progress_updater(chat_id, reply_to, stop_event, label="Processing"):
    try:
        percent = 0
        bars = 12
        bar_empty = "░"
        bar_full = "█"
        try:
            progress_msg = bot.send_message(chat_id, f"{label}: 0% [{bar_empty * bars}]", reply_to_message_id=reply_to)
        except:
            progress_msg = None
...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        ...
        progress_thread = threading.Thread(target=fake_progress_updater, args=(chat_id, data.get("origin", call.message.message_id), stop_event, log_action), daemon=True)
...
def handle_media(message):
    ...
    try:
        ...
        def transcribe_and_send():
            stop_event = threading.Event()
            progress_thread = threading.Thread(target=fake_progress_updater, args=(message.chat.id, message.message_id, stop_event, "Transcribing"), daemon=True)
            progress_thread.start()
            try:
                try:
